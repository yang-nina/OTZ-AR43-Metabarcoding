...
## taxonomy
taxonomy <- read_qza("../00_qiime2-outputs/18S/ar43-18s-prdb-taxonomy_hybrid_classify.qza")
head(taxonomy$data)
...
# This shows there are 16 potential contaminants out of 9777 sequences.
...
After extracting the contaminants, the fasta files can be blasted to identify the top matches. I used Geneious Prime to blast the sequences. I created a new AR43-18S folder and imported the contaminants fasta file. 
...
blast <- read.table("../00_qiime2-outputs/18S/ar43-18s-prdb-taxonomy_hybrid_classify.tsv", sep='\t', header=T, row.names=1, comment = "")
head(blast)
colnames(blast)
head(blast)
dim(blast)
...
write.csv(join_df, "18S/prot_blast-contaminants-seqs-prev05_18s_hybrid.csv", row.names = TRUE)
