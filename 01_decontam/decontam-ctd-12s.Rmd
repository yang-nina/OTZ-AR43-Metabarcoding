...
## Background
This document outlines the stephyseq taken to decontaminate 12S metabarcoding results from samples collected via CTD from the AR43 Cruise (Armstrong, March 2020) using the decontam package in R. The resulting project is a decontaminated phyloseq objective for protists only.
...
## hybrid taxonomy
tax_hybrid <- read_qza("../00_qiime2-outputs/12S/ar43-12s-taxonomy_hybrid_classify.qza")
head(tax_hybrid$data)
strsplit_output <- (strsplit(as.character(tax_hybrid$data$Taxon), ";"))
...
tax_table_hybrid <- do.call(rbind, list_of_vectors)
colnames(tax_table_hybrid) <- c("Domain", "Phylum", "Class", "Order", "Family", "Genus", "Species")
head(tax_table_hybrid)
rownames(tax_table_hybrid) <- tax_hybrid$data$Feature.ID
head(tax_table_hybrid)

physeq_hybrid <- phyloseq(
  otu_table(ASVs$data, taxa_are_rows = T),
  tax_table(tax_table_hybrid),
  sample_data(metadata)
)

physeq_hybrid # check that this is a phyloseq object
rank_names(tax_table_hybrid, errorIfNULL=TRUE)
tax_table_hybrid # 876 taxa 
...
df <- as.data.frame(sample_data(physeq_hybrid))
df$LibrarySize <- sample_sums(physeq_hybrid)
...
ggsave("12S/librarysize_samples_prot_hybrid.pdf", height = 4, width = 5, units = "in", dpi=300)
...
sample_data(physeq_hybrid)$is.neg <- sample_data(physeq_hybrid)$Type == "control"
contamdf.prev <- isContaminant(physeq_hybrid, method="prevalence", neg="is.neg")
...
contamdf.prev05 <- isContaminant(physeq_hybrid, method="prevalence", neg="is.neg", threshold=0.5)
...
# 0.5 threshold
ps.nc <- prune_taxa(!contamdf.prev05$contaminant, physeq_hybrid)
ps.c <- prune_taxa(contamdf.prev05$contaminant, physeq_hybrid)
...
ggsave("12S/contam_prevalence_tresholds_12s_hybrid.pdf", height = 3, width = 4, units = "in", dpi=300)
...
head(df)
summary(df)
write.csv(df, "12S/contaminants-seqs-ctd-prev05_12s_hybrid.csv", row.names = TRUE)
...
writeLines(extracted_sequences, "12S/decontam-extracted-seqs-contaminants-prev05_12s_hybrid.fasta")
...
freq <- read.csv("../00_qiime2-outputs/12S/ar43-12s-feature-frequency-detail.csv")
head(freq)
# rename columns
colnames(freq);
...
prev05 <- read.csv("12S/contaminants-seqs-ctd-prev05_12s_hybrid.csv")
...
## I used the taxonomy file to identify these contaminants (can also use Geneious?)
blast <- read.table("../00_qiime2-outputs/12S/ar43-12s-taxonomy_hybrid.tsv", sep='\t', header=T, row.names=1, comment = "")
head(blast)
dim(blast)
class(blast)
rownames(blast)
...
head(blast)
colnames(blast)
blast_df <- as.data.frame(blast[c(2:877), c(1:5)])
summary(blast_df)
head(blast_df)
...
write.csv(join_df, "12S/blast-contaminants-seqs-prev05_12s_hybrid.csv", row.names = TRUE)
...
physeq_hybrid
ps.noncontam <- prune_taxa(!contamdf.prev05$contaminant, physeq_hybrid)
ps.noncontam
