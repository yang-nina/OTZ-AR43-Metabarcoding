...
## Background
This document outlines the steps taken to decontaminate 18S V9 metabarcoding results from samples collected via CTD from the AR43 Cruise (Armstrong, March 2020) using the decontam package in R. The resulting project is a decontaminated phyloseq objective for metqzoans (mostly invertebrates) only.
...
The details of the QIIME 2 steps were performed on WHOI's HPC.
...
## taxonomy
taxonomy <- read_qza("../00_qiime2-outputs/18S/ar43-18s-silvadb-taxonomy-naivebayes.qza")
head(taxonomy$data)
...
# The library sizes of the positive samples primarily fall from 2,000 to 100,000 reads. The negative control samples have the lowest reads as expected. Note: It is important keep the low-read samples for now, because we want to use those negative controls to help identify contaminants!
...
# This shows there are 5 potential contaminants out of 499 sequences.
...
ggsave("18S/contam_prevalence_thresholds_met.pdf", height = 3, width = 4, units = "in", dpi=300)
