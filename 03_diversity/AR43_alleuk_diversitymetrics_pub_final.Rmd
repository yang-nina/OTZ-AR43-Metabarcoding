...
classifier <- read_csv("classifier_stats.csv") # pulled this from the qiime2 outputs for evaluating classifiers
head(classifier)
...
# 2. I will run a script to extract sequences for the unknown feature IDs based against a reference fasta (12s-vert-ctd-rep-seqs.fasta - created during decontam)
...
prot_rarefaction <- prot_sampling_coverage %>% 
  ggplot(aes(x=1, y = n_seqs)) + 
  geom_jitter(fill = "#78C18A", size = 1, color = "black", shape = 21) + theme_light() +
  scale_y_log10() + # a handful of samples below 10,000   
  labs(title = NULL,
       y = "Sequences",
       x = NULL) +
...
head(rare_prot)
# let's join to get depth

prot_specaccum_mat <- rare_prot %>%
...
sac_prot <- specaccum(decostand(prot_specaccum_mat, method = "pa"),
                    method = "random", permutations = 999)
prot_specaccum_df <- tibble(
  samples = sac_prot$sites,
  richness = sac_prot$richness,
  sd = sac_prot$sd
)

head(prot_specaccum_df)

ggplot(prot_specaccum_df, aes(samples, richness)) +
...
rare_prot_depth <- rare_prot %>%
  left_join(metadata_18s %>% select(sample, depth), by = "sample") %>%
  filter(!is.na(depth))
head(rare_prot_depth)

run_sac_depth <- function(dat, perms = 999) {
  ...
  # need >= 2 samples to accumulate
  if (nrow(prot_mat) < 2) return(NULL)

  sac <- specaccum(decostand(prot_mat, method = "pa"),
                   method = "random",
                   permutations = perms)
  tibble(
    ...
    sd       = sac$sd,
    ymin     = pmax(richness - sd, 0),
    ymax     = richness + sd
  )
}

prot_specaccum_by_depth <- rare_prot_depth %>%
  split(.$depth) %>%
  purrr::map_dfr(run_sac_depth, perms = 999)

prot_specaccum_by_depth <- prot_specaccum_by_depth %>%
  mutate(depth = factor(depth, levels = sort(unique(as.numeric(depth)))))

prot_specaccum_plot <- ggplot(prot_specaccum_by_depth,
       aes(x = samples, y = richness, color = depth, fill = depth)) +
  geom_ribbon(aes(ymin = ymin, ymax = ymax), alpha = 0.15, colour = NA) +
  geom_line(linewidth = 0.5) +
  scale_y_continuous(limits = c(0, 3000), breaks = seq(0, 3000, by = 1000), expand = c(0, 0)) +
  scale_x_continuous(limits = c(0, 13), breaks = seq(0, max(prot_specaccum_by_depth$samples), by = 2), expand = c(0, 0)) + 
  labs(x = "Number of samples",
       y = "ASV richness",
       color = "Depth (m)",
       fill  = "Depth (m)") +
...
met_rarefaction <- met_sampling_coverage %>% 
  ggplot(aes(x=1, y = n_seqs)) + 
  geom_jitter(fill =  "#FE869B", size = 1, color = "black", shape = 21) + theme_light() +
  scale_y_log10() + # a handful of samples below 10,000   
  labs(title = NULL,
       y = "Sequences",
       x = NULL) +
...
run_sac_depth <- function(dat, perms = 999) {
  ...
  # need >= 2 samples to accumulate
  if (nrow(met_mat) < 2) return(NULL)

  sac <- specaccum(decostand(met_mat, method = "pa"),
                   method = "random",
                   permutations = perms)
...
met_specaccum_by_depth <- rare_met_depth %>%
  split(.$depth) %>%
  purrr::map_dfr(run_sac_depth, perms = 999)

met_specaccum_by_depth <- met_specaccum_by_depth %>%
  mutate(depth = factor(depth, levels = sort(unique(as.numeric(depth)))))


met_specaccum_plot <- ggplot(met_specaccum_by_depth,
       aes(x = samples, y = richness, color = depth, fill = depth)) +
  geom_ribbon(aes(ymin = ymin, ymax = ymax), alpha = 0.15, colour = NA) +
  geom_line(linewidth = 0.5) +
  scale_x_continuous(limits = c(0, 13), breaks = seq(0, max(met_specaccum_by_depth$samples), by = 2), expand = c(0, 0)) + 
  labs(x = "Number of samples",
       y = "ASV richness",
       color = "Depth (m)",
       fill  = "Depth (m)") +
...
fish_rarefaction <- fish_sampling_coverage %>% 
  ggplot(aes(x=1, y = n_seqs)) + 
  geom_jitter(fill =  "#1986be", size = 1, color = "black", shape = 21) + theme_light() +
  scale_y_log10() + # a handful of samples below 10,000   
  labs(title = NULL,
       y = "Sequences",
       x = NULL) +
...
rare_fish_depth <- rare_fish %>%
  left_join(metadata_12s %>% select(sample, depth), by = "sample") %>%
  filter(!is.na(depth))
head(rare_fish_depth)
...
run_sac_depth <- function(dat, perms = 999) {
  ...
  # need >= 2 samples to accumulate
  if (nrow(fish_mat) < 2) return(NULL)

  sac <- specaccum(decostand(fish_mat, method = "pa"),
                   method = "random",
                   permutations = perms)
...
fish_specaccum_by_depth <- rare_fish_depth %>%
  split(.$depth) %>%
  purrr::map_dfr(run_sac_depth, perms = 999)

fish_specaccum_by_depth <- fish_specaccum_by_depth %>%
  mutate(depth = factor(depth, levels = sort(unique(as.numeric(depth)))))


fish_specaccum_plot <- ggplot(fish_specaccum_by_depth,
       aes(x = samples, y = richness, color = depth, fill = depth)) +
  geom_ribbon(aes(ymin = ymin, ymax = ymax), alpha = 0.15, colour = NA) +
  geom_line(linewidth = 0.5) +
  scale_x_continuous(limits = c(0, 13), breaks = seq(0, max(fish_specaccum_by_depth$samples), by = 2), expand = c(0, 0)) + 
  labs(x = "Number of samples",
       y = "ASV richness",
       color = "Depth (m)",
       fill  = "Depth (m)") +
...
prot_rarefaction <- prot_rarefaction + ggtitle("Protists (18S)")
met_rarefaction <- met_rarefaction + ggtitle("Invertebrates (18S)")
fish_rarefaction <- fish_rarefaction + ggtitle("Vertebrates (12S)")

rarefaction_panel <- prot_rarefaction + met_rarefaction + fish_rarefaction +
...
ggsave("./plots-final/rarefaction_panel_revisionon1.pdf", height = 80, width = 180, units = "mm", dpi=300)


# Species accumulation panels

prot_specaccum_plot <- prot_specaccum_plot + ggtitle("Protists (18S)")
met_specaccum_plot <- met_specaccum_plot + ggtitle("Invertebrates (18S)")
fish_specaccum_plot <- fish_specaccum_plot + ggtitle("Vertebrates (12S)")

specaccum_panel <- prot_specaccum_plot + met_specaccum_plot + fish_specaccum_plot +
#  plot_annotation(tag_levels = "A", tag_suffix = '.') + 
  plot_layout(guides = 'collect') & 
    theme(panel.border = element_rect(linewidth = 0.5),
...
ggsave("./plots-final/specaccum_panel_revisionon1.pdf", height = 75, width = 180, units = "mm", dpi=300)
...
alpha_obs <- ggplot(obs_df, aes(y = fct_rev(as.factor(depth)), x = alpha_div)) +
  ...
  labs(title = "Richness",
       y = "Depth (m)",
       x = NULL) +
  scale_fill_manual(values = group_colors, name = NULL) + theme_custom_paper() +  
  theme(
...
ggsave("plots-final/alpha_div_indice_panel_pub_revision1.pdf", height = 130, width = 80, units = "mm", dpi=300)
# a legend was added using Affinity Designer
...
# let's look at diversity casts for signs of higher species richness at night
fish_taxa_order %>% select(depth, deployment, TAXA) %>% unique() %>% 
  group_by(depth, deployment) %>%
  summarize(
    n_unique_taxa = n_distinct(TAXA),
    .groups = "drop"
  )

fish_species_count <- fish_taxa_order %>% select(depth, deployment, Species) %>% unique() %>% 
    filter(!is.na(Species),
         !Species %in% c("Unassigned Actinopterygii", "Unassigned Perciformes"))

head(fish_species_count)
write.csv(fish_species_count, "fish_species_count.csv")

fish_species_sum <- fish_taxa_order %>% 
  filter(!is.na(Species),
         !Species %in% c("Unassigned Actinopterygii", "Unassigned Perciformes")) %>%
  select(depth, deployment, Species) %>% unique() %>% 
  #group_by(depth, deployment, Species) %>%
  group_by(depth, deployment) %>%
  summarize(
...
fish_species_count_wide <- fish_species_sum %>%
  pivot_wider(
    names_from  = deployment,
    values_from = n_unique_species,
    values_fill = 0
  )

head(fish_species_count_wide)
...
save(prot_taxa, met_taxa_order2, fish_taxa_order, file = "./psmelt_tables_for_networks.RData")
...
ggsave("./plots-final/prot_relabun_deploy_barplot.pdf", height = 3, width = 6, units = "in", dpi=300)
...
ggsave("./plots-final/met_relabun_deploy_barplot.pdf", height = 3, width = 6, units = "in", dpi=300)

length(unique(met_taxa_order3$OTU)) # 399


# let's extract only the salp sequences:

salp_df <- met_taxa_order3 %>% group_by(depth, deployment) %>%
    mutate(RelativeAbundance = (Abundance / sum(Abundance)) * 100, na.rm = TRUE) %>%
    mutate(depth = as.factor(depth)) %>%
    ungroup() %>%
    group_by(depth, deployment, TAXA) %>%
    ...
    arrange(depth, desc(sum_RelativeAbundance)) %>%
    mutate(TAXA = fct_reorder(TAXA, sum_RelativeAbundance, .desc = FALSE)) %>%
  filter(TAXA == "Tunicata-Salpida")

head(salp_df)

salp_df %>% 
  ggplot() +
  geom_col(aes(y = fct_rev(depth), x = sum_RelativeAbundance), fill = "#1ec38c", color = "black", width = 0.8, linewidth = 0.5) +
  labs(title = NULL,
       y = "Depth (m)",
       x = "Relative Abundance (%)") +
  facet_wrap(. ~ deployment) + 
...
ggsave("./plots-final/salp_relabun_deploy_barplot.pdf", height = 3, width = 6, units = "in", dpi=300)
...
# let's calculate the number of TAXA for each depth:
...
ggsave("./plots-final/fish_relabun_deploy_barplot.pdf", height = 3, width = 6, units = "in", dpi=300)
...
```{r relabun-plots-by-replicate, eval = TRUE, message = FALSE, warning = FALSE}

make_taxabar_relabun_deploy_rep <- function(df) {
  df %>%
    # ---- FIX: enforce numeric ordering of depth_rep ----
    mutate(
      depth_num = as.integer(str_extract(depth_rep, "^[0-9]+")),
      rep_num   = as.integer(str_extract(depth_rep, "(?<=_)[0-9]+")),
      depth_rep = fct_reorder(depth_rep, depth_num * 100 + rep_num)
    ) %>%
    # ----------------------------------------------------
    group_by(depth_rep, deployment) %>%
    mutate(RelativeAbundance = (Abundance / sum(Abundance)) * 100, na.rm = TRUE) %>%
    mutate(depth = as.factor(depth)) %>%
    ungroup() %>%
    group_by(depth_rep, deployment, TAXA) %>%
    summarise(sum_RelativeAbundance = sum(RelativeAbundance, na.rm = TRUE), .groups = "drop") %>%
    arrange(depth_rep, desc(sum_RelativeAbundance)) %>%
    mutate(TAXA = fct_reorder(TAXA, sum_RelativeAbundance, .desc = FALSE)) %>%
    ggplot(aes(y = fct_rev(depth_rep), x = sum_RelativeAbundance)) +
    geom_col(aes(fill = TAXA), color = "black", width = 0.8, linewidth = 0.5) +
    labs(title = NULL,
         y = "Depth (m)",
         x = "Relative Abundance (%)") +
...
head(prot_taxa2)
colnames(prot_taxa2)
prot_taxa2 <- prot_taxa2 %>%
  mutate(depth_rep = paste(depth, rep, sep = "_"))
...
prot_relabun_rep <- make_taxabar_relabun_deploy_rep(prot_taxa2) + 
  theme(legend.position = "bottom") +
      labs(x = "Relative Abundance (%)",
       y = "Depth (m)", 
       title = "Protists (18S)") + 
...
ggsave("./plots-final/prot_relabun_rep_barplot.pdf", height = 4, width = 8, units = "in", dpi=300)


# Invertebrates

head(met_taxa_order3)
colnames(met_taxa_order3)
met_taxa_order3 <- met_taxa_order3 %>%
  mutate(depth_rep = paste(depth, rep, sep = "_"))
...
met_relabun_rep <- make_taxabar_relabun_deploy_rep(met_taxa_order3) + 
  labs(x = "Relative Abundance (%)",
       y = "Depth (m)", 
       title = "Invertebrates (18S)") + 
  theme(legend.position = "bottom") +
...
ggsave("./plots-final/met_relabun_rep_barplot.pdf", height = 4, width = 8, units = "in", dpi=300)

# let's extract by salps
salp_df <- met_taxa_order3 %>%
    group_by(depth_rep, deployment) %>%
    mutate(RelativeAbundance = (Abundance / sum(Abundance)) * 100, na.rm = TRUE) %>%
    mutate(depth = as.factor(depth)) %>%
    ungroup() %>%
...
head(fish_taxa_order2)
colnames(fish_taxa_order2)
fish_taxa_order2 <- fish_taxa_order2 %>%
  mutate(depth_rep = paste(depth, rep, sep = "_"))
...
fish_relabun_rep <- make_taxabar_relabun_deploy_rep(fish_taxa_order2) + 
  labs(x = "Relative Abundance (%)",
       y = "Depth (m)", 
       title = "Vertebrates (12S") + 
  theme(legend.position = "right") +
...
ggsave("./plots-final/fish__relabun_rep_barplot.pdf", height = 4, width = 8, units = "in", dpi=300)
...
depth_colors <- c("#fde725", "#b5de2b", "#35b779", "#26828e", "#3e4989", "#440154")
...
ggsave("./plots-final/beta_diversity_pca_all_pub.pdf", height = 62, width = 180, units = "mm", dpi=300)
...
ggsave("./plots-final/beta_diversity_pca_all_revised_pub.pdf", height = 62, width = 180, units = "mm", dpi=300)
...
```{r beta-stats-cast, eval = TRUE, message = FALSE, warning = FALSE}
